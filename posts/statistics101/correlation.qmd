...
# Define a custom template (without scaleanchor in template)
custom_template = {
    "data": {
        "scatter": [
            {
...
pio.templates["custom_scatter"] = custom_template
...
fig.update_layout(
    template="custom_scatter",
    xaxis=dict(
        scaleanchor="y",  # Lock the scale of x-axis to the y-axis
        scaleratio=1,  # Set the ratio (1:1 for equal scale)
    ),
    yaxis=dict(
        scaleanchor="x",  # Lock the scale of y-axis to the x-axis
        scaleratio=1,  # Set the ratio (1:1 for equal scale)
    ),
)
fig.show()
...
fig.update_layout(template="custom_scatter", title='相関係数水準に応じた回帰係数と逆回帰係数の比較')
fig.update_xaxes(
